...
function calc_inss($valor)
{
    return round((float) $valor * 0.08, 2);
}
function calc_irrf($base)
{
    return round((float) $base * 0.075, 2);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'print') {
        ...
        foreach ((array)($_POST['salario_base'] ?? []) as $id => $val) {
            ...
            $hnt = floatval(str_replace(['.',','],['', '.'], $_POST['horas_nao_trabalhadas'][$id] ?? 0));
            $vhn = floatval(str_replace(['.',','],['', '.'], $_POST['valor_horas_nao_trabalhadas'][$id] ?? 0));
            ...
            $descContr = floatval(str_replace(['.',','],['', '.'], $_POST['desc_contrato'][$id] ?? 0));
            $liq  = floatval(str_replace(['.',','],['', '.'], $_POST['salario_liquido'][$id] ?? 0));
            $db->query(
                "UPDATE folha_pagamento SET salario_base={$sb},horas_trabalhadas={$ht},horas_nao_trabalhadas={$hnt},valor_horas_nao_trabalhadas={$vhn},horas_extras={$he},valor_extras={$ve},outros_descontos={$outros},desc_contrato={$descContr},desconto_inss={$inss},desconto_irrf={$irrf},salario_liquido={$liq} WHERE id={$id}"
            );
        }
                ...
                <tbody>
                    <?php foreach ($lista as $f):
                        ...
                        <tr>
                            ...
                            <td><?= $f['status'] ?></td>
                            <td>
                                <button type="button" onclick="updateAndPrint(<?= $f['id'] ?>)" style="padding:0 4px;">📄</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
...
<?php endforeach; ?>
    ...
    <script>
        ...
        function updateAndPrint(id) {
            const f = document.getElementById('print' + id);
            const get = n => document.querySelector(`[name='${n}[${id}]']`);
            f.querySelector('[name="salario_base"]').value = get('salario_base').value;
            f.querySelector('[name="horas_extras"]').value = get('horas_extras').value;
            f.querySelector('[name="inss"]').value = get('desconto_inss').value;
            f.querySelector('[name="irrf"]').value = get('desconto_irrf').value;
            f.querySelector('[name="outros_descontos"]').value = get('outros_descontos').value;
            f.querySelector('[name="salario_liquido"]').value = get('salario_liquido').value;
            f.submit();
        }
    </script>
</body>
